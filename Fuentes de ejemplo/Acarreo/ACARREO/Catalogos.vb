...
Public Class Catalogos
    ...
    Private Sub sp1_DialogKey(ByVal sender As Object, ByVal e As FarPoint.Win.Spread.DialogKeyEventArgs) Handles sp1.DialogKey
        ...
        ElseIf e.KeyData = Keys.Enter And sender.Equals(sp1) And opc = 7 Then
            If sp1.Sheets(0).ActiveColumnIndex = 2 Then
                InsertaEtiqueta()
            End If
        ElseIf e.KeyData = Keys.Enter And sender.Equals(sp1) And opc = 10 Then
            If sp1.Sheets(0).ActiveColumnIndex = 2 Then
                InsertaChofer()
            End If
        ...
        ElseIf e.KeyData = Keys.F9 And sender.Equals(sp1) And opc = 7 Then
            If sp1.Sheets(0).Cells(sp1.Sheets(0).ActiveRowIndex, 0).Text <> "" Then
                ...
                End If
            End If
        ElseIf e.KeyData = Keys.F9 And sender.Equals(sp1) And opc = 10 Then
            If sp1.Sheets(0).Cells(sp1.Sheets(0).ActiveRowIndex, 0).Text <> "" Then
                If MsgBox("Desea eliminar el chofer seleccionado?", MsgBoxStyle.YesNo, "Eliminar") = MsgBoxResult.Yes Then
                    Try
                        reg.Open("delete from ChoferesAcarreo where codigo = " & sp1.Sheets(0).Cells(sp1.Sheets(0).ActiveRowIndex, 0).Text, cna, ADODB.CursorTypeEnum.adOpenStatic)
                    Catch ex As Exception
                        Exit Sub
                    End Try
                    sp1.Sheets(0).RemoveRows(sp1.Sheets(0).ActiveRowIndex, 1)
    ...
    Sub Lotes()
        ...
        sp1.Sheets(0).ColumnHeader.Cells(0, 2).Text = "HAs."
        ...
        'ALTURA DE FILAS
        sp1.ActiveSheet.ColumnHeader.Rows(0).Height = 40

        sp1.Sheets(0).Columns(0, sp1.ActiveSheet.Columns.Count - 1).AllowAutoSort = True
    ...
    Sub Choferes()
        ...
        sp1.Sheets(0).Columns(0).CellType = tcell2
        Try
            reg.Open("select Codigo, Nombre, Domicilio from ChoferesAcarreo order by Nombre ASC", cna, ADODB.CursorTypeEnum.adOpenStatic)
        Catch ex As Exception
            Exit Sub
        End Try
        While Not reg.EOF
            For R As Integer = 0 To 2
                If IsDBNull(reg.Fields(R).Value) Then
                    sp1.Sheets(0).Cells(cont, R).Text = String.Empty
                Else
                    sp1.Sheets(0).Cells(cont, R).Text = reg.Fields(R).Value.ToString.ToUpper
                End If
            Next
            reg.MoveNext()
            cont = cont + 1
        ...
        sp1.Size = New Size(486, 379)
        sp1.Location = New Point(90, 20)
        sp1.Sheets(0).ColumnHeader.Cells(0, 0).Text = "No.*".ToUpper
        sp1.Sheets(0).ColumnHeader.Cells(0, 1).Text = "Nombre*".ToUpper
        sp1.Sheets(0).ColumnHeader.Cells(0, 2).Text = "Domicilio".ToUpper
    ...
    Private Sub InsertaChofer()
        ...
        End If
        If sp1.Sheets(0).Cells(sp1.Sheets(0).ActiveRowIndex, 2).Text <> "" Then
            Domicilio = sp1.Sheets(0).Cells(sp1.Sheets(0).ActiveRowIndex, 2).Text
        End If
        Try
            reg.Open("select codigo, Nombre, Domicilio from ChoferesAcarreo where codigo = " & Codigo, cna, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
        Catch ex As Exception
            Exit Sub
        End Try
        If reg.EOF Then
        ...
        End If
        reg.Fields(0).Value = Codigo
        reg.Fields(1).Value = isnull(nombre, String.Empty).ToUpper
        reg.Fields(2).Value = Domicilio
        reg.Update()
        reg.Close()
    ...
    Private Sub sp1_CellDoubleClick(ByVal sender As Object, ByVal e As FarPoint.Win.Spread.CellClickEventArgs) Handles sp1.CellDoubleClick
        If opc = 1 Then
            If IsNumeric(sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text) Then
                Principal.txtIdProductor.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text
                Principal.txtProductor.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 1).Text
                Me.Close()
            End If
        ElseIf opc = 2 Then
            If IsNumeric(sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 1).Text) Then
                Principal.txtIdProducto.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 1).Text
                Principal.txtProducto.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 2).Text
                Me.Close()
            End If
        ElseIf opc = 5 Then
            If IsNumeric(sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text) Then
                Principal.txtIdLote.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text
                Principal.txtLote.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 1).Text
                Me.Close()
            End If
        ElseIf opc = 10 Then
            If IsNumeric(sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text) Then
                Principal.txtIdChofer.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 0).Text
                Principal.txtChofer.Text = sp1.ActiveSheet.Cells(sp1.ActiveSheet.ActiveRowIndex, 1).Text
                Me.Close()
            End If
        End If
    End Sub
