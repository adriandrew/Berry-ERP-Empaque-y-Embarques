...
#Region "MÉTODOS"
    Sub LlenaLotes()
        ...
        cmd = New OleDbCommand("Select Codigo as Id, Descri as Descripcion from Lote order by Codigo" & _
                               " Union Select -1, 'TODOS' from Lote", cnn)
    ...
    Sub LlenaGrupoLotes()
        ...
        Dim adap As New OleDb.OleDbDataAdapter
        Dim rst As New DataSet
        cmd = New OleDbCommand("Select Codigo as Id, Nombre as Descripcion from grupolote order by Codigo" & _
                               " Union Select -1, 'TODOS' from Lote", cnn)
        adap.SelectCommand = cmd
        Try
            adap.Fill(rst)
    ...
    Sub FormatoReporte()
        ...
        spReporte.Sheets(0).ColumnCount = 7
        ...
        spReporte.Sheets(0).Columns(2).Tag = "productor"
        spReporte.Sheets(0).Columns(3).Tag = "lote"
        spReporte.Sheets(0).Columns(4).Tag = "producto"
        spReporte.Sheets(0).Columns(5).Tag = "cajas"
        spReporte.Sheets(0).Columns(6).Tag = "peso"
        ...
        'ANCHO DE COLUMNAS
        spReporte.Sheets(0).Columns("folio").Width = 80
        spReporte.Sheets(0).Columns("fecha").Width = 80
        spReporte.Sheets(0).Columns("productor").Width = 250
        spReporte.Sheets(0).Columns("lote").Width = 100
        spReporte.Sheets(0).Columns("producto").Width = 150
        spReporte.Sheets(0).Columns("cajas").Width = 80
        spReporte.Sheets(0).Columns("peso").Width = 80
        ...
        'ANCHO DE COLUMNAS
        spReporte.Sheets(1).Columns("productor").Width = 250
        spReporte.Sheets(1).Columns("lote").Width = 100
        spReporte.Sheets(1).Columns("producto").Width = 150
        spReporte.Sheets(1).Columns("cajas").Width = 80
        spReporte.Sheets(1).Columns("peso").Width = 80
    ...
    Sub ReporteAcarreo(ByVal lote As Integer, ByVal desde As String, ByVal hasta As String)
        ...
        cmd = New OleDbCommand("select acarreo.folio,acarreo.fecha, embarcador.nombre, lote.descri, producto.descex, sum(acarreo.cajas) as sumcajas, " & _
        ...
        " group by acarreo.folio,acarreo.fecha, embarcador.nombre, lote.descri, producto.descex " & _
        " order by acarreo.fecha, acarreo.folio ", cnn)
        adap.SelectCommand = cmd
        Try
            adap.Fill(rst)
        Catch ex As Exception
    ...
    Sub ReporteTotales(ByVal lote As Integer, ByVal desde As String, ByVal hasta As String)
        ...
        " order by embarcador.codigo, lote.codigo ", cnn)
        adap.SelectCommand = cmd
        Try
            adap.Fill(rst)
        Catch ex As Exception
        ...
        " from acarreo " & _
        " where folio <> '' " & _
        ArmaCondicion(lote, desde, hasta), cnn)
        adap.SelectCommand = cmd
        Try
            adap.Fill(rst)
        Catch ex As Exception
    ...
    Sub ReporteRendimientos(ByVal grupolote As Integer, ByVal desde As String, ByVal hasta As String)
        ...
        'PROPIEDADES GENERALES DEL SPREAD
        cmd = New OleDbCommand("select  grupolote.nombre, palets.producto, producto.descex, sum(iif(tipo = 2, sumbultos, 0)) as cajasacarreo, sum(iif(tipo = 1, sumbultos, 0)) as bultospalets, " & _
        ...
        " from (select sum(palets.bultos) as sumbultos, sum(palets.peso*palets.bultos) as sumpeso, grupolote.nombre, producto.descex, palets.lote, palets.producto, palets.fechaempaque as fecha,  " & _
        " 1 as tipo " & _
        " from palets, lote, grupolote, producto " & _
        " where palets.lote = lote.codigo " & _
        " and palets.producto = producto.codigo " & _
        " and lote.grupo = grupolote.codigo " & _
        ArmaCondicionRendimientosPallets(grupolote, desde, hasta) & _
        " group by palets.lote, palets.productor, palets.producto, palets.fechaempaque, grupolote.nombre, palets.lote, producto.descex " & _
        " UNION " & _
        " select sum(acarreo.cajas) as sumbultos, sum(acarreo.peso) as sumpeso, grupolote.nombre, producto.descex, acarreo.lote, acarreo.producto, acarreo.fecha, 2 as tipo  " & _
        " from acarreo, lote, grupolote, producto " & _
        " where acarreo.lote = lote.codigo " & _
        " and lote.grupo = grupolote.codigo " & _
        " and acarreo.producto = producto.codigo " & _
        ArmaCondicionRendimientosAcarreo(grupolote, desde, hasta) & _
        " group by acarreo.lote, acarreo.productor, acarreo.producto, acarreo.fecha,  grupolote.nombre, acarreo.lote, producto.descex) as tabla1 " & _
        " group by grupolote.nombre, palets.producto, producto.descex " & _
        " order by grupolote.nombre ", cnn)
        adap.SelectCommand = cmd
        Try
            adap.Fill(rst)
        Catch ex As Exception
    ...
    Function ArmaCondicionRendimientosPallets(ByVal grupolote As Integer, ByVal desde As String, ByVal hasta As String)
        Dim condicion As String = ""
        If grupolote > -1 Then
            condicion = " and lote.grupo = " & grupolote
        End If
